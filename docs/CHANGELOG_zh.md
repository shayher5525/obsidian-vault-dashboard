...
[English](../CHANGELOG.md) · **中文** · [日本語](CHANGELOG_ja.md) · [한국어](CHANGELOG_ko.md) · [Français](CHANGELOG_fr.md) · [Español](CHANGELOG_es.md) · [Italiano](CHANGELOG_it.md)
...
- 新外观 **蔚蓝天空**（`nio`）。依 `AA_WorkMaterial 工作资料/91_DesignFiles 设计文件/2026-09-02_蔚来风格视觉设计规范.md` v1.0（源出公开展示的《NIO 视觉识别系统第1.4版》手册内页）建立，只借「青绿 Teal 主强调 + 蓝倾向品牌白 / 夜空黑基底 + 同族蓝绿明度分层」这套视觉语法，不使用蔚来标志、Blue Sky Coming 标语与 Blue Sky 定制字体，字体按规范走 Montserrat / Inter / 思源黑体替代栈。一屏一个强调色：浅色取 Tonal Teal `#004B64`（白底约 7.0:1），深色取 Teal `#00BEBE`（黑底约 8.3:1）；青绿 Teal `#00BEBE` 只作图形级强调——进度条、热力峰值、2px 下划线滑块。大面用 Soft Blue 与蓝倾向品牌白，灰阶只留在文字；中圆角 8—16px + 低透明度柔影；热力图只走 Teal 单色明度阶。主标签选中态是 2px 青绿下划线滑块，工厂快捷标签是带细描边的胶囊、选中转青绿描边与文字。
- 离线测试新增十五道蔚蓝天空守卫：色板闭集、八个基准色必声明、阴影原色 ≤ 0.30 低透明度、主标签 2px 青绿下划线、热力峰值取 Teal、圆角 ≤ 16px、`nio` id 与 8 语显示名注册齐备。42/42 通过。
...
- 流行快餐浅色态：热力图里没有活动的日子完全看不见。空活动格与其身后的容器底同为浅金 `#FFF4D6`——同一个值，对比度 1.00:1。空活动格改用 White，既保住「金成面」的那块金底，又读作在金底上开的一个口。深色态本就可分辨，不动。

## [0.15.0] — 2026-09-02

### 新增

- 新外观 **黑场光点**（`sony`）。依 `AA_WorkMaterial 工作资料/91_DesignFiles 设计文件/2026-09-02_索尼风格视觉设计规范.md` v1.0（源出 2007 年索尼美国识别手册）建立，只借「黑场基底、开放框、几何无衬线、单点色光」这套视觉语法，不使用索尼标志、标语与商业授权字体，ITC Avant Garde 按规范改走 Poppins / Jost / Questrial / 思源黑体替代栈。基底是纯黑 `#000000` 而非柔和黑；深度只由 `#000000` → `#0D0D0D` → `#1A1A1A` 三阶明度表达，不用投影；圆角归零，控件至多 2px。强调色一屏只用一个：深色取 ocean `#00A4E8`（黑底约 7.4:1），浅色取 slate `#165B65`。浅色不沿用 ocean，因为它在白底仅约 2.7:1，扛不住文字——规范要求浅色态复核对比度，而不是把深色配色取反。强调色不作大色块，故选中态是 2px 滑动下划线而非填充胶囊，卡片数字保持黑白，唯一着色的文字是链接。深色是规范正稿，浅色走规范自身为文字密集页定的反相口径。
- 离线测试新增五道黑场守卫，逐条对应规范硬规则：色板闭集、禁用 `rgba()` 调色（色须 100% 使用）、无投影、圆角 ≤ 2px、八个基准色必须声明。
...
- 设置页文案不再引用作者自己的库。「迭代日志」改为按「适合谁、怎么算」描述（「适合在库里维护 Markdown 变更日志的人，按日期小节解析条目数」），不再写「本库原有行为」——那句话对别人毫无意义。三处带同样假设的代码注释一并清理：插件无构建步骤，`main.js` 就是用户看到的源码。
- 按 README 的语种补齐译文变更记录，置于 `docs/`，并在各语种 README 正文加入对应语种的链接。根目录 `CHANGELOG.md` 改为英文，与 `README.md` 一致。
